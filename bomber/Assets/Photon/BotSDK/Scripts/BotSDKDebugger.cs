...
public unsafe class BotSDKDebugger : QuantumCallbacks, IBotDebug
{
  ...
  private bool TrySetupUTDebugger()
  {
    var frame = QuantumRunner.Default.Game.Frames.Predicted;
    if (frame.Has<UTAgent>(_prefabView.EntityRef) == false)
    {
      return false;
    }

    _utAgent = frame.Get<UTAgent>(_prefabView.EntityRef);
    return true;
  }
  ...
  public string GetUTRootName()
  {
    if (_utAgent.Equals(default) == true && TrySetupUTDebugger() == false)
    {
      return "";
    }

    return _utAgent.UtilityReasoner.UTRoot.Id != default ? UnityDB.FindAsset<UTRootAsset>(_utAgent.UtilityReasoner.UTRoot.Id).name : "";
  }
}
