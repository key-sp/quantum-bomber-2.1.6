...
namespace Quantum {
  ...
  public sealed class UnityResourceLoader : IResourceLoader, IDisposable {
    ...
    private void FinishLoading(UnityAssetResource resource, AssetBase asset) {
      ...
      } else if (resource.AssetWrapper != null) {
        // seems the asset has already been loaded; this may happen if there are multiple
        // calls before the first LoadSync returns
        Debug.Assert(resource.StateValue >= AssetResourceState.Loaded);
      } else {
