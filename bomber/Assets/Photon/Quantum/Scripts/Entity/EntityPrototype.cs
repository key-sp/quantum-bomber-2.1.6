...
public class EntityPrototype : MonoBehaviour, IQuantumPrefabNestedAssetHost {
  ...
  void DrawGizmos(bool selected) {
    if (!QuantumGameGizmos.ShouldDraw(QuantumEditorSettings.Instance.DrawColliderGizmos, selected)) {
      return;
    }
    ...
    // handle overriding from components
    {
      ...
      if (TransformMode == EntityPrototypeTransformMode.Transform2D || transform2D != null) {
        ...
        var cc = SafeGetPrototype(GetComponent<EntityComponentCharacterController2D>());
        if (cc != null) {
          UnityDB.TryFindAssetObjectForInspector(cc.Config, out configCC2D);
        }
      }
      ...
      if (TransformMode == EntityPrototypeTransformMode.Transform3D || transform3D != null) {
        ...
        var cc = SafeGetPrototype(GetComponent<EntityComponentCharacterController3D>());
        if (cc != null) {
          UnityDB.TryFindAssetObjectForInspector(cc.Config, out configCC3D);
        }
      }
    }

    var style = QuantumEditorSettings.Instance.ColliderGizmosStyle;
    ...
    if (config2D != null) {
      ...
      if (config2D.ShapeType == Shape2DType.Polygon) {
        ...
        if (collider) {
          QuantumGameGizmos.DrawShape2DGizmo(Shape2D.CreatePolygon((PolygonCollider)collider.AssetObject, config2D.PositionOffset, FP.FromRaw((config2D.RotationOffset.RawValue * FP.Raw.Deg2Rad) >> FPLut.PRECISION)), position2D, rotation2DDeg.ToUnityQuaternionDegrees(), 
        ...
        foreach (var shape in config2D.CompoundShapes) {
          ...
          if (shape.ShapeType == Shape2DType.Polygon) {
            ...
            if (collider) {
              QuantumGameGizmos.DrawShape2DGizmo(Shape2D.CreatePolygon(collider.Settings, shape.PositionOffset, FP.FromRaw((shape.RotationOffset.RawValue * FP.Raw.Deg2Rad) >> FPLut.PRECISION)), position2D, rotation2DDeg.ToUnityQuaternionDegrees(), 
                color, height, null, style: style);
            }
          }
          else {
    ...
    if (configCC2D != null) {
      QuantumGameGizmos.DrawCharacterController2DGizmo(position2D, configCC2D, selected, style: style);
    }
    ...
    if (configCC3D != null) {
      QuantumGameGizmos.DrawCharacterController3DGizmo(position3D, configCC3D, selected, style: style);
    }
  }
