...
public sealed class QuantumRunner : MonoBehaviour, IDisposable {
  ...
  public struct StartParameters {
    ...
    /// <summary>
    /// By default, a Quantum session creates one or more additional frame instances to cache previous states that can
    /// be used for interpolation, notably for transform interpolations on the View.
    /// Set this parameter to true if you want to disable this behaviour (e.g. a server-side or console-only simulation),
...
#if !QUANTUM_DISABLE_RUNNER_FIX
    // Moved here from Dispose(), because on a plugin disconnect (CallbackPluginDisconnect) Dispose() will be called which removed the runner from the 
    // static registry right away making it unable to be found by ShutdownAll() leaving a dormant runner object in the scene.
    // The correct was is to register to CallbackPluginDisconnect and Shutdown(false) the runner during the callback. 
    // To disable the fix add QUANTUM_DISABLE_RUNNER_FIX to the Unity scripting defines (to all required platforms).
  ...
  public void Shutdown() {
    // Runner is shut down, destroys its gameobject, will trigger OnDisable(), which will destroy the session, finally session will call dispose on the runner.
    Destroy(gameObject);
  }

  public void Dispose() {
    // Called by the Session.Destroy(). This is called when the session has an internal error (CallbackPluginDisconnect).
#if QUANTUM_DISABLE_RUNNER_FIX
    _activeRunners.Remove(this);
#endif
  }
  ...
  public static QuantumRunner StartGame(String clientId, StartParameters param) {
    ...
    int gameFlags = 0;
    if (param.DisableInterpolatableStates) {
    ...
    args.DisableInterpolatableStates = param.DisableInterpolatableStates;
