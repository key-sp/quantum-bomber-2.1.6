...
namespace Quantum.Editor {
  ...
  public class AssetDBInspector : EditorWindow {
    ...
    private static readonly Dictionary<string, GUIContent> _iconCache = new Dictionary<string, GUIContent>();
    private static readonly int SearchFieldHashCode = "ToolbarSearchField".GetHashCode();
    ...
    private void UpdateEntries() {
      ...
      foreach (var resource in resources) {
        ...
        var resourceGuid = resource.Guid.ToString();

        if (string.IsNullOrEmpty(_search) == false) {
          if (filename.IndexOf(_search, StringComparison.OrdinalIgnoreCase) < 0 &&
              resourceGuid.IndexOf(_search, StringComparison.OrdinalIgnoreCase) < 0) { 
            continue;
          }
        }
        ...
        _entries.Add(new Entry {
          Resource = resource,
          Filename = filename,
          FormatedPath = resourcePath,
          ResourceType = resourceType,
          ResourceLabel = resourceLabel,
          GuidAsString = resourceGuid
        });
      }
    }
    ...
    public void OnGUI() {
      if (_isDirty || _entries == null) {
        _isDirty = false;
        UpdateEntries();
      }

      using (new GUILayout.HorizontalScope()) { 
        if (GUILayout.Button(TryLoadIcon("TreeEditor.Refresh", "<>", "Refresh list of resources"), GUILayout.Width(30) )) {
          UpdateEntries();
        }

        if (QuantumRunner.Default == null && GUILayout.Button(new GUIContent("Generate Quantum Asset DB", "Write all Quantum assets into AssetResources.asset"))) {
        ...
        using (new EditorGUI.DisabledScope(QuantumRunner.Default != null)) {
          if (GUILayout.Button(new GUIContent("Export", "Export Asset DB as a Json file"), GUILayout.Width(50))) {
        ...
        { 
          _search = UnityInternal.EditorGUI.ToolbarSearchField(SearchFieldHashCode, EditorGUILayout.GetControlRect(GUILayout.Width(200)).AddY(3), _search, false);
      ...
      using (var scrollView = new GUILayout.ScrollViewScope(_scrollPos)) {
        ...
        foreach (var e in _entries) {
          ...
          var loaded = e.Resource.IsLoaded;
          GUI.color = loaded ? Color.green : Color.gray;
          EditorGUI.LabelField(rect, TryLoadIcon("blendSampler", "O", null));

          GUI.color = loaded ? Color.white : Grey;
          if (GUI.Button(rectGuid, e.GuidAsString, GUI.skin.label)) {
            GenericMenu menu = new GenericMenu();
            menu.AddItem(new GUIContent("Copy"), false, g => GUIUtility.systemCopyBuffer = (string)g, e.Resource.Guid.ToString(false));
            menu.AddItem(new GUIContent("Copy As Long"), false, g => GUIUtility.systemCopyBuffer = (string)g, e.Resource.Guid.Value.ToString());
            menu.ShowAsContext();
          }

          var color = e.ResourceType == ResourceType.View ? Color.cyan : e.ResourceType == ResourceType.Prototype ? Color.yellow : Color.green;

          GUI.color = loaded ? Desaturate(color, 0.75f) : Desaturate(color, 0.25f);
          if (GUI.Button(rectName, new GUIContent(e.ResourceLabel, e.Resource.Path), GUI.skin.label)) {
            var selectCandidates = AssetDatabase.FindAssets(Path.GetFileName(e.FormatedPath)); //, new string[] { $"Assets/Resources/{Path.GetDirectoryName(resource.URL)}"});

            var candidateGuid = string.Empty;
            switch (e.ResourceType) {
              case ResourceType.Asset:
                candidateGuid = Array.Find(selectCandidates, c => AssetDatabase.GUIDToAssetPath(c).EndsWith($"{e.FormatedPath}.asset"));
                break;
              case ResourceType.Prototype:
              case ResourceType.View:
                candidateGuid = Array.Find(selectCandidates, c => AssetDatabase.GUIDToAssetPath(c).EndsWith($"{e.FormatedPath}.prefab"));
                break;
            }
          ...
          if (GUI.Button(rectButton, loaded ? "Dispose" : "Load")) {
            if (loaded) {
              UnityDB.DefaultResourceManager.DisposeAsset(e.Resource.Guid);
              UnityDB.DefaultResourceManager.MainThreadSimulationEnded();
              _isDirty = HideUnloaded;
              return;
            } else {
              UnityDB.DefaultResourceManager.GetAsset(e.Resource.Guid, true);
            }
          }
        }
    ...
    private static GUIContent TryLoadIcon(string iconname, string replacement, string tooltip) {
      if (_iconCache.ContainsKey(iconname)) {
        return _iconCache[iconname];
      }

      var result = default(GUIContent);
      if (EditorGUIUtility.Load(iconname) as Texture2D == null) {
        result = new GUIContent(replacement, tooltip);
      } else {
        result = EditorGUIUtility.IconContent(iconname, tooltip);
        if (result.image == null) {
          result.text = replacement;
        }
      }

      _iconCache.Add(iconname, result);
      return result;
    }
  }
}
