...
namespace Quantum.Editor {
  ...
  internal class QuantumCodeIntegrationVisualStudio : AssetPostprocessor {
    ...
    private static string PatchSolution(string solutionPath, string solutionContent) {
      ...
      // enforce Windows path separator
      quantumCodePackagesDirectory = quantumCodePackagesDirectory.Replace('/', '\\');
...
#if QUANTUM_VSINTEGRATION_ENFORCE_RELEASE_CONFIGURATION
      if (!unitySoltution.Configurations.ContainsKey("Release|Any CPU")) {
        unitySoltution.Configurations.Add("Release|Any CPU", "Release|Any CPU");
        foreach (var project in unitySoltution.Projects) {
          project.Configurations.Add("Release|Any CPU", new Dictionary<string, string>() {
